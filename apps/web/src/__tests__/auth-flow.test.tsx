...
global.fetch = (async (input: RequestInfo, init?: RequestInit): Promise<Response> => {
  ...
  const bodyRaw: unknown = init?.body ? JSON.parse(init.body as string) : {};
  const body: { email?: string } = (
    typeof bodyRaw === 'object' && bodyRaw !== null ? bodyRaw : {}
  ) as { email?: string };
  ...
  if (url.endsWith('/auth/refresh')) {
    return new Response(JSON.stringify({ accessToken: 'at2', refreshToken: 'rt2' }), {
      status: 200,
    });
  }
